...
export function getVotingTokenMint(
  ...
  return findProgramAddress(
    [poolId.toBuffer(), Buffer.from("voting_mint_seed", "utf8")],
    programId
  );
}
...
export function getVoterAddress(
  ...
  return findProgramAddress(
    [registrar.toBuffer(), Buffer.from("voter", "utf8"), authority.toBuffer(), ],
    programId
  );
}
...
export function getTokenOwnerRecordAddress(
  ...
  return findProgramAddress(
    [Buffer.from("governance", "utf8"), realm.toBuffer(), governingTokenMint.toBuffer(), governingTokenOwner.toBuffer(), ],
    programId
  );
}
